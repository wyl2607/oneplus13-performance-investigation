...
# ab-real-workload.sh
#
# Real-workload A/B: gzip -9 over a fixed 908 MiB input, run under an app uid,
# stock versus uclamp.max held at 1024.
#
...
# usage: sh ab-real-workload.sh [arm ...]   e.g. `sh ab-real-workload.sh A B` to smoke test
[ -n "$*" ] && ORDER="$*"
...
for ARM in $ORDER; do
  ...
  echo "  pinned: node6=$PN6 p6max=$PP6 p0max=$PP0"
  if [ "$PP6" != "$TARGET_PRIME" ] || [ "$PP0" != "$TARGET_MID" ]; then
    echo "  !! CEILING NOT PINNED - run marked INVALID"
    echo "RUN|$RUN|$ARM|INVALID|ceiling_not_pinned|p0max=$PP0|p6max=$PP6" >> $OUT/summary.txt
    continue
  fi
  ...
  STATS=$(awk -F, -v tp="$TARGET_PRIME" -v tm="$TARGET_MID" 'NR>1 {
      n++;
      if ($3==6 || $3==7) prime++;
...
awk -F'|' '$4=="OK" {
  ...
  END {
    ...
    if (n["A"]>0 && n["B"]>0) {
      ma=tot["A"]/n["A"]; mb=tot["B"]/n["B"];
      printf "\nA/B wall-time ratio = %.3f  (B faster by %.1f%%)\n", mb/ma, (ma-mb)*100/ma;
    }
  }' $OUT/summary.txt
