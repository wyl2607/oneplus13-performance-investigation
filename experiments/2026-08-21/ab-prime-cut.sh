#!/system/bin/sh
# A/B, alternating, for the one result worth acting on.
#
# cool-shape.sh measured 2438400/2400000 at 1050 work against the daily level's
# 2841600/2400000 at 1003 -- +4.7 % for the same temperature, from LOWERING the
# prime ceiling by two OPP steps. That is a claim about which cluster should get
# the shared power budget, and it is worth a conf change if it holds. But +4.7 %
# sits right on this device's ~5 % run-to-run spread, and cool-shape ran each
# point once, in a fixed order, with start temperatures 2 C apart.
#
# So: two runs each, alternating A/B/A/B so that any drift over the session (the
# device soaking, the battery draining) splits evenly between the two arms, and a
# tighter cooldown gate (46 C, minimum 120 s) so the points start from the same
# thermal state rather than merely a legal one.
...
COOL_P6=2649600
COOL_P0=2227200
OUT=/data/local/tmp/ab-prime-cut.txt
LOCK=/data/local/tmp/ab.lock
...
say "BARE DEVICE, 8 workers (one per core), ${DUR}s per point, alternating A/B/A/B"
say "A = daily level as shipped (prime 2841600).  B = prime cut to 2438400."
point A1 2841600 2400000 96000 || exit 1
point B1 2438400 2400000 96000 || exit 1
point A2 2841600 2400000 96000 || exit 1
point B2 2438400 2400000 96000 || exit 1
say ""
say "=== done ==="
