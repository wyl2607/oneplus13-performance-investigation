...
EXTREME_P6=3513600
# 中核走额定 3532800，不是 2918400。2026-09-21 A/B 实测（DATA.md 44 节，接散热器，
# 超大核与红线都不动，只动这一个值）：多核 8027 -> 8305，+3.48%，t=4.66；单核
# 2224 -> 2232，不动（+0.34%，t=0.45），符合「中核与单核无关」。机制闭合：中核
# at-ceiling 从 53.8% 掉到 0.0-0.4%，交付频率 2.617 -> 2.749 GHz，+3.2~5.0%——
# 上限不再是中核的活跃约束，所以 3532800 之上没有东西可拿（它就是额定）。
# 热代价实测为零：结温峰值 89.6 -> 90.3 C（红线 92），三轮零退让，外壳反而更凉。
# **只改 3 档**。1/2 档是裸机档，本轮证据全部来自接散热器的 3 档，拿它去改裸机档
# 就是重犯 42 节「用八核满载证据改少核档位」的错。
EXTREME_P0=3532800
EXTREME_GATE=92000
EXTREME_TIMEOUT=0
